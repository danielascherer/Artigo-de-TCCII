...
Na primeira etapa foi estudado o domínio do problema e as técnicas que foram empregadas em sua solução. Tendo em vista a diversidade de dispositivos que possibilitam a navegação na internet, e a necessidade da utilização da aplicação sem realizar a instalação, foi decidido  na web em um site responsivo. 
...
Após a análise das características do domínio do problema optou-se pelo desenvolvimento de um sistema especialistas para a primeira funcionalidade. 


no qual  serão apresentadas uma série de proposições as quais a usuária irá responder conforme suas vivências e ao final, os resultados de suas respostas são associados à Lei Maria da Penha, se for o caso.





 Além disso, o sistmea apresentará um nível de segurança sobre o relacionamento podendo ser; relacionamento seguro, fique atenta a atitudes abusivas, você está envolvida em um relacionamento abusivo ou você está em situação de violência procure ajuda. 

A segunda funcionalidade, de apoio à denúncia, será vinculada automaticamente as respostas apresentadas pela primeira funcionalidade quando identificados altos níveis de violência em um relacionamento, caso contrário poderá ser acessada em outra aba, como um espaço para informação a respeito dos procedimentos de denùncia de situações de violência, localizar serviços especializados no atendimento à mulher e telefones úteis. 

Considerando que a aplicação visa a preservação da identidade da utilizadora não serão realizados cadastros para utilização das funcionalidades da aplicação.

\subsubsection{Aquisição de Conhecimento}

A etapa de aquisição do conhecimento será realizada com as técnicas manuais baseadas em descrições, abordadas na seção 2.3.2.2, pois parte do conhecimento necessário para atingir os objetivos do sistema especialista encontra-se na lei 11.340/06 descrita na seção 2.2, e em bibliografias revisadas na seção 2.1. 

Para complementar esta etapa, será utilizado o conhecimento de um especialista humano da àrea do atendimento especializado à mulher, sobre como identificar se uma mulher está vivenciando um relacionamento abusivo. Para adquirir este conhecimento, será utilizada a técnica baseada em entrevista semi estruturada. 

\subsubsection{Implementação}

Esta etapa é dedicada à construção do sistema especialista proposto juntamente com a representação formal do conhecimento adquirido através da ferramenta de engenharia de conhecimento. 
